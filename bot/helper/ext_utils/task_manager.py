...
from .quota_manager import is_premium as _is_premium_user
    ...
    async with queue_dict_lock:
        ...
        if (
            ...
            if is_over_limit:
                event = Event()
                if state == "dl":
                    queued_dl[listener.mid] = {"event": event, "user_id": listener.user_id}
                else:
                    queued_up[listener.mid] = {"event": event, "user_id": listener.user_id}
...
async def start_dl_from_queued(mid: int):
    (queued_dl[mid]["event"] if isinstance(queued_dl[mid], dict) else queued_dl[mid]).set()
    del queued_dl[mid]
    non_queued_dl.add(mid)


async def start_up_from_queued(mid: int):
    (queued_up[mid]["event"] if isinstance(queued_up[mid], dict) else queued_up[mid]).set()
    del queued_up[mid]
...
async def _ordered_keys(dct):
    ...
    try:
        ...
        for k in keys:
            ...
            prem = await _is_premium_user(uid) if uid else False
            enriched.append((k, prem))
        # premium first; stable sort
        enriched.sort(key=lambda x: (not x[1], ))
        return [k for k,_ in enriched]
    except Exception:
        return keys
...
async def start_from_queued():
        ...
        async with queue_dict_lock:
            ...
            if all_ < all_limit:
                ...
                if queued_up and (not up_limit or up < up_limit):
                    for index, mid in enumerate(await _ordered_keys(queued_up), start=1):
                        await start_up_from_queued(mid)
                        f_tasks -= 1
                        if f_tasks == 0 or (up_limit and index >= up_limit - up):
                            break
                if queued_dl and (not dl_limit or dl < dl_limit) and f_tasks != 0:
                    for index, mid in enumerate(await _ordered_keys(queued_dl), start=1):
    ...
    if up_limit := Config.QUEUE_UPLOAD:
        ...
        async with queue_dict_lock:
            if queued_up:
                for mid in await _ordered_keys(queued_up):
                    await start_up_from_queued(mid)
    ...
    if dl_limit := Config.QUEUE_DOWNLOAD:
        ...
        async with queue_dict_lock:
            if queued_dl:
                for mid in await _ordered_keys(queued_dl):
                    await start_dl_from_queued(mid)
