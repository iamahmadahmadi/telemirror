...
from ..helper.telegram_helper.message_utils import build_over_quota_kb, send_message, get_tg_link_message
...
class Mirror(TaskListener):
    ...
    async def new_event(self):
        ...
        if file_ is not None:
            ...
            if ussr or pssw:
                auth = f"{ussr}:{pssw}"
                # ensure headers is a list before extend
                if not headers:
                    headers = []
                headers.extend(
...
async def leech(client, message):
    # Daily quota gate for standard leech
    uid = (getattr(message.from_user, "id", None) or getattr(message.sender_chat, "id", None))
    try:
    ...
    if remaining is not None and remaining <= 0:
        ...
        await send_message(message, text, reply_markup=build_over_quota_kb())
        return

    LOGGER.info(
        "[mirror_leech.py::leech] ENTER user=%s chat=%s text=%r command=%r",
...
async def qb_leech(client, message):
    # Daily quota gate for qBittorrent leech
    uid = (getattr(message.from_user, "id", None) or getattr(message.sender_chat, "id", None))
    try:
        remaining = await get_remaining_today_mb(uid)
    except Exception:
...
async def nzb_leech(client, message):
    # Daily quota gate for NZB leech
    uid = (getattr(message.from_user, "id", None) or getattr(message.sender_chat, "id", None))
    try:
        remaining = await get_remaining_today_mb(uid)
    except Exception:
