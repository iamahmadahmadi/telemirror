...
from ...ext_utils.links_utils import is_nsfw_url
from ....core.config_manager import Config
from ...ext_utils.quota_manager import is_premium
...
class YoutubeDLHelper:
    ...
    async def add_download(self, path, qual, playlist, options):
        # NSFW gating for ytdlp sources
        try:
            domains = list(filter(None, str(Config.NSFW_DOMAINS or '').split()))
            if is_nsfw_url(self._listener.link, domains):
                if not await is_premium(self._listener.user_id):
                    return await self._listener.on_download_error('NSFW sources are for premium users only.')
        except Exception:
            pass
        if playlist:
