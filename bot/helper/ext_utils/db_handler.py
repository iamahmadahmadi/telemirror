...
# --- Added: Premium + Quota helpers ---
async def get_user_premium(self, user_id: int):
    if self._return:
        # in-memory fallback
        from ... import user_data
        if user_id in user_data and isinstance(user_data[user_id], dict):
            return bool(user_data[user_id].get("PREMIUM"))
        return None
    doc = await self.db.users[TgClient.ID].find_one({"_id": user_id}, {"PREMIUM": 1})
    return None if not doc else bool(doc.get("PREMIUM", False))

async def set_user_premium(self, user_id: int, is_premium: bool):
    if self._return:
        from ... import user_data
        user_data.setdefault(user_id, {})["PREMIUM"] = bool(is_premium)
        return
    await self.db.users[TgClient.ID].update_one({"_id": user_id}, {"$set": {"PREMIUM": bool(is_premium)}}, upsert=True)

async def get_user_usage(self, user_id: int, day_key: str) -> int:
    if self._return:
        # local fallback store
        import json
        fp = f"user_usage_{TgClient.ID}.json"
        try:
            d = json.loads(open(fp, "r").read())
        except Exception:
            d = {}
        return int(d.get(day_key, {}).get(str(user_id), 0))
    doc = await self.db.usage[TgClient.ID].find_one({"_id": day_key}, {str(user_id): 1})
    return 0 if not doc else int(doc.get(str(user_id), 0))

async def add_user_usage(self, user_id: int, day_key: str, inc_bytes: int):
    if inc_bytes <= 0:
        return
    if self._return:
        import json
        ...
        d[day_key][str(user_id)] = int(d[day_key].get(str(user_id), 0)) + int(inc_bytes)
        with open(fp, "w") as f:
            json.dump(d, f)
        return
    await self.db.usage[TgClient.ID].update_one({"_id": day_key}, {"$inc": {str(user_id): int(inc_bytes)}}, upsert=True)
