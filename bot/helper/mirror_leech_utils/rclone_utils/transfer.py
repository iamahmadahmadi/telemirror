...
class RcloneTransferHelper:
    ...
    async def download(self, remote, config_path, path):
        ...
        # Apply performance tuning flags when downloading from Google Drive.  If
        # custom flags have been specified via the listener (rc_flags), those
        # take precedence.  Otherwise we consult the global Config values.  See
        # Config.RCLONE_DOWNLOAD_* variables for details.
        if remote_type == "drive" and not self._listener.rc_flags:
            # Always acknowledge potential abuse files to avoid interactive prompts.
            cmd.extend(("--drive-acknowledge-abuse",))

            # Chunk size for drive downloads.
            chunk_mb = getattr(Config, "RCLONE_DOWNLOAD_DRIVE_CHUNK_SIZE", 0)
            if isinstance(chunk_mb, (int, str)):
                try:
            ...
            if chunk_mb and chunk_mb > 0:
                cmd.extend(("--drive-chunk-size", f"{chunk_mb}M"))
            else:
                # fallback to previous default of 128M to improve performance
                cmd.extend(("--drive-chunk-size", "128M"))

            # Concurrency and rate limits
            transfers = getattr(Config, "RCLONE_DOWNLOAD_TRANSFERS", 1)
            tpslimit = getattr(Config, "RCLONE_DOWNLOAD_TPSLIMIT", transfers)
            tpsburst = getattr(Config, "RCLONE_DOWNLOAD_TPS_BURST", transfers)
            try:
    ...
    async def upload(self, path):
        ...
        # Apply performance tuning flags for Google Drive uploads.  Unless the
        # caller specified custom rclone flags via rc_flags, we look up the
        # values from Config.  See Config.RCLONE_UPLOAD_* variables for more
        # information.
        if remote_type == "drive" and not self._listener.rc_flags:
            # Chunk size for drive uploads
            chunk_mb = getattr(Config, "RCLONE_UPLOAD_DRIVE_CHUNK_SIZE", 0)
            if isinstance(chunk_mb, (int, str)):
                try:
                    chunk_mb = int(chunk_mb)
                except Exception:
            ...
            # Concurrency and rate limits
            transfers = getattr(Config, "RCLONE_UPLOAD_TRANSFERS", 1)
            tpslimit = getattr(Config, "RCLONE_UPLOAD_TPSLIMIT", transfers)
            tpsburst = getattr(Config, "RCLONE_UPLOAD_TPS_BURST", transfers)
            try:
                transfers = int(transfers)
            except Exception:
                transfers = 1
